...
def new_ticket():
    ...
    if form.validate_on_submit():
        ...
        # Классификатор / начальник может создать заявку от имени другого пользователя
        # (например, если обращение поступило по телефону)
        applicant_id = current_user.id
        on_behalf_of = request.form.get("on_behalf_of", type=int)
        if on_behalf_of and current_user.role in ["classifier", "head", "admin"]:
