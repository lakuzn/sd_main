...
import { setActiveMode } from '../ticket/statusManager.js';
    ...
    const socket = initSocket(ticketId, {
        ...
        onStatusChanged: (data) => {
            if (data.ticket_id == ticketId) {
                if (data.new_status === 'Решена') {
                    // Заявку закрыли: перезагружаем, чтобы обновились статус,
                    // доступные действия и кнопки ("Открыть повторно" / "Удалить заявку")
