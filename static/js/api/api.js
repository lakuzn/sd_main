...
export async function fetchResolveTicket(ticketId, buttonElement) {
    ...
    try {
        ...
        const response = await fetch(`/api/ticket/${ticketId}/resolve`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': csrfToken
        ...
        alert(`Не удалось закрыть заявку: ${error}`);
        return null;
    } finally {
        if (buttonElement) {
            buttonElement.disabled = false;
