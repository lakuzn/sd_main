...
class TicketService:
    ...
    def determine_applicant(current_user, on_behalf_of_id):
        """
        Определение заявителя с учётом права "от имени".
        Возвращает (applicant_id, error_message)
        """
        if not on_behalf_of_id:
            return current_user.id, None

        # Проверка прав на создание "от имени"
    ...
    def update_user_hostname(user):
        """Обновление hostname пользователя по IP"""
    ...
    def get_ticket_context(ticket_id, current_user, form=None):
        ...
        if current_user.role != "user":
            ...
            context["executors"] = User.query.filter(
                User.role.in_(["executor", "classifier", "head", "admin"])
            ).all()
